...
#### `self.next\_level()` Behavior (Overridable)

By default, calling `self.next\_level()` will:

* Increment `\_current\_level\_index` by 1 on `frame + 1` (the frame after `self.next\_level()` is called)
...
* Restart the current level (via `level\_reset()`) if any actions have been taken
* Otherwise, perform a full game reset (via `full\_reset()`)

Both `level\_reset()` and `full\_reset()` can be overridden with custom logic.
