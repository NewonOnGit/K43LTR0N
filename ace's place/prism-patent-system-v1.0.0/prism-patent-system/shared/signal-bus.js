...
class SignalBus {
  ...
  onPrefix(prefix, handler) {
    ...
    // Patch emit to also call prefix handlers
    // (We store on original emit and check prefixes)
    return () => {
  ...
  // Override emit to also check prefix subscriptions
  /**
   * Internal: check prefix subscriptions after standard emit.
   * This is handled by wrapping the original emit.
   */
