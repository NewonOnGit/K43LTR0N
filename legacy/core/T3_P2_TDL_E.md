...
**Document Status:** Layer 3 document (Tier 3, Projection 2). Complete P2 paper with distributed CLOSURE and COMPUTATIONAL_COMPLEXITY content. The h-direction in sl(2,ℝ), orbit type det > 0, Δ > 0 (hyperbolic), constant e = exp(1).
...
**What this paper does NOT contain:** The full Dist derivation (Paper 1), bridge chain (Paper 2A), complete {R,N} algebra (Paper 2B), P1/P3-specific content (Papers 3-P1, 3-P3), cross-projection synthesis (Paper 3-META), or physics interpretations (Paper 6B).
...
P2 is the analysis of self-relating difference in its transport mode (Paper 0 §1½): R viewed as level-transition between tower strata. Every result in this paper traces to the exponential flow exp(th), where h is the Cartan element determined by R and N. The constant e measures the rate of emergence: how quickly R's self-action populates new tower levels.
...
**Remark (e is not the circle number).** The constant e enters Euler's identity not because it "knows about circles" but because the exponential map is the universal completion law for generators. The circle appears because the generator i has purely imaginary eigenvalues, forcing the exponential to trace S¹. For a different generator, exp produces a different geometry — exp(th) traces a hyperbola. The fact that exp converts both generators into their natural flows is P2's content: emergence/realization as a universal operation.

**Remark (TDL Emergence as Forced Derivation).** Emergence in the TDL sense is constructive: the exponential map exhibits step-by-step the structure that is algebraically implicit in the generator. det(exp(R)) = e is not the creation of a new constant — e was always the determinant of R's exponential. The bridge chain (Paper 2 Thm 2.1) does not generate novelty; it unfolds what {0,1} already contained with zero branching at every step. The appropriate verb is "derive," not "emerge" in the philosophical sense: TDL emergence is the canonical exhibition of implicit structure via the unique (zero-branching) path. The Mediating Act (P2 meta-primitive) balances and transitions — it does not create from nothing. What looks like "new area" (det = e > 1) is the realization of area that was always latent in R's trace: tr(R) = 1, so the area factor exp(1) = e is determined by the integer entry structure of R.
...
**P2 contains I² (via emergence path).** The emergence path 1 → p₁ → p₁p₂ → ... → n builds n from 1 via prime multiplication. Each step multiplies by a prime — this IS the I² operation (f acting on g → f·g) applied at the number level. Building n from 1 via prime multiplication IS I²-composition applied level by level.
...
The anti-TDL replaces exponential growth 𝒰 = ×e with exponential decay 𝒰 = ×e⁻¹. The level structure persists but inverts: emergence becomes reduction, and vice versa.
...
V_T(n) measures the level gap between emergence depth (how many prime factors built n) and reduction depth (how many digit-sum steps reach a single digit). V_T(1) = |0−0| = 0 (fixed point: zero prime factors, already single digit).
