...
**Remark (Ch-Maj Gap as O+/O− Observable in SHA-256).** In SHA-256 proof-of-work mining at difficulty d, the Hamming weight gap Ch(w₀,w₁,w₂) − Maj(w₀,w₁,w₂) on the upper output triple scales linearly: Gap ≈ 0.285 × d (Paper T-COMP Thm C.19). The mechanism: difficulty forces w₀ → 0, and Ch(0,w₁,w₂) = w₂ (preserves information, O− face) while Maj(0,w₁,w₂) = w₁&w₂ (destroys information, O+ face). This is the structural invertibility threshold measured at the application level: below some difficulty, O+ and O− are balanced; above, O− (selection/observation) dominates O+ (consensus/production). The gap is the continuous observable converting a threshold condition (hash < target) into a direct measurement of the P3/P1 asymmetry through the Cl(1,1) Boolean functions that define the O+/O− split.
