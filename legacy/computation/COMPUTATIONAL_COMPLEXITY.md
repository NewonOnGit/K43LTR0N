...
## Companion to: COMPUTATIONAL_PRIMITIVES.md (Part I), LAMBDA_PRIME_LATTICE.md, KMS_SELECTION_THEOREM.md

**Abstract.**
We develop the complete complexity-theoretic structure of the five-primitive computational
framework derived in Part I. The signature system assigns to every algorithm a 4-vector
...
via the Lucas semiring, and the Gödel algorithm showing incompleteness of Alg. All
numerical claims are computationally verified. A single conjecture remains open (Conjecture
10.4 of Part I: exact MIX threshold under complexity assumptions).
...
| Exact MIX threshold = φ̄²/2 | **Conjecture** | OWF existence or P ≠ NP |
...
*See also: COMPUTATIONAL_PRIMITIVES.md (Part I — five atomic types, HALT=GapP, MIX bounds,
S₃ duality gaps); LAMBDA_PRIME_LATTICE.md (C_max bound, Zeckendorf structure);
KMS_SELECTION_THEOREM.md (Z(β)=coth(β/2)⁴, shell counts, selection theorem);
THREE_PROJECTIONS_UNIFIED.md (bridge chain, P1–P3 forcing).*
